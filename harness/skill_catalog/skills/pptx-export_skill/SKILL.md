---
name: pptx
description: Erzeugt/liest .pptx (Decks, Layouts, Speaker-Notes). Trigger: Slides/Deck/Praesentation.
metadata:
  catalog_id: pptx-export_skill
  author: Anthropic
...
# Praesentationen erstellen  (`pptx-export_skill`)

> **AEGIRA-Katalog-Referenz.** Dies ist die kuratierte Katalog-Karte fuer den Skill `pptx`.
...
## Zweck
Erzeugt/liest .pptx (Decks, Layouts, Speaker-Notes). Trigger: Slides/Deck/Praesentation.
...
1. Quelle abrufen: `https://github.com/anthropics/skills` (Pfad `pptx/SKILL.md`).
2. Frontmatter `name`/`description` validieren; Lizenz pruefen.
3. `content_sha256` berechnen und ins `_manifest.json` schreiben.
4. Inhalt unveraendert nach `.claude/skills/pptx/SKILL.md` ins Harness-ZIP.
